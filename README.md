...
[![Methodology](https://img.shields.io/badge/methodology-v5.29-green.svg)](https://magistra.health/en/methodology)
...
Live `clinical` block for nausea from the request above, re-captured by re-running the request above against production on 2026-09-18 (the trial-registry re-collection recorded in "Corrections in v5.23", and STEP 3 entering ten effects' bases on 2026-09-18, grew the profile-matched nausea base from 41 rates/26 sources at the 2026-09-09 capture to 128/26) (`basisNl`, the Dutch twin of `basis`, omitted for brevity; re-captured after the v5.4 interval correction — see "Corrections in v5.4" in the preprint; the interval is now anchored at the pooled rate, so it differs from the 2026-08-31 capture; `pooledPercentage` was added 2026-08-28 and the base counts have grown with the corpus):

```json
{
  "percentage": 45,
  "confidenceInterval": { "low": 20, "high": 73 },
  "confidenceLevel": "very_high",
  "sourceDiversity": "very_high",
  "dataPointCount": 182,
  "ratePointCount": 128,
  "rateSourceCount": 26,
  "basis": "128 stated rates from 26 distinct sources (of 182 clinical/regulatory records matching this profile's sex and dose tier). Base rate 21% → 45% after profile adjustment (sex:female ×1.25, isFirstMonth ×2.5) — odds ratios hand-coded at the 2026-04-12 seed with no per-modifier citation recorded, not derived from this corpus",
  "isFallback": false,
...
Note what the response discloses about itself: the pre-adjustment rate (21%), every modifier applied to reach 45%, and the fact that those odds ratios are hand-coded rather than fitted from this corpus. A wide interval (20–73) is not a formatting artefact — it is the honest spread of 128 rates from 26 sources, and since v5.4 its width is fixed by that evidence: the same profile-free interval (7–46 around the 21% pooled rate, verified against production 2026-09-18 by re-running the request with no modifiers set) is carried, on the log-odds scale, to wherever the modifiers move the centre.
...
- **No formal calibration; the fit runs but is degenerate.** As of 2026-09-18, 12 of the 15 tracked effects clear the ≥10-eligible-rate minimum and are fitted daily — but both covariates the model fits are constant across the whole eligible base (all 1,111 eligible rate points behind those fits carry `sex: "unspecified"`, and none an age band including 65), so every coefficient fits at 0 with an odds ratio of 1. The binding constraint is the absence of demographic covariates on the rows eligible to be fitted, not an eligible base too small to fit at all; this bullet read "Planned at n ≥ 500" until 2026-09-18, which named the wrong constraint. See §4 of the methodology paper and "Corrections in v5.28".
...
**Goyal, S.** (2026). *A Dual-Track Framework for GLP-1 Side Effect Estimation: Separating Clinical Evidence from Real-World Patient Reports* (v5.29). Magistra, Phlo Systems BV. https://magistra.health/en/methodology
